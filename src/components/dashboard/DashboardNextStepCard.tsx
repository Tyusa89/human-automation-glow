...
import { resolveDashboardNextStep } from "@/lib/templates/resolveDashboardNextStep";
...
export function DashboardNextStepCard({ activeTemplateSlug }: Props = {}) {
  ...
  const step = resolveDashboardNextStep(signals, activeTemplateSlug ?? null);

  // Power users with nothing to do: hide entirely
  if (tier === "power" && step.kind === "nothing" && !activeTemplateSlug) return null;

  // If truly nothing and no template, hide
  if (step.kind === "nothing" && !activeTemplateSlug) return null;
