import { Section, Feature } from "@/components/Sections";
import { Shield, Database, Globe } from "lucide-react";

export default function TrustPage() {
  return (
    <Section title="Trust & Security" eyebrow="Governance first">
      <div className="grid md:grid-cols-3 gap-4">
        <Feature icon={<Shield className="h-5 w-5" />} title="Compliance" copy="SOC 2 Type II, GDPR, data residency options." />
        <Feature icon={<Database className="h-5 w-5" />} title="Data Controls" copy="Row-Level Security, role-based access, PII redaction, secrets vault." />
        <Feature icon={<Globe className="h-5 w-5" />} title="Deployment" copy="Multi-tenant cloud, VPC peering, or self-hosted for maximum control." />
      </div>
    </Section>
  );
}